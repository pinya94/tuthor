// ── Examen de Música (Pentagrama Path) ───────────────────────────────────────
// Versión estructurada y graduada de Pentagrama Path: 4 fases con progresión
// (localStorage), cada una superable con un 70% de notas correctas. Dos modos
// según la fase:
...
//
// El modo libre/arcade (partitura infinita) vive en /juegos/pentagrama-path
// (Modo Survivor) — este archivo es solo el examen tipo "ronda graduada".
...
export default function MusicaExamen() {
  ...
  useEffect(() => {
    ...
    saveActivity(user.uid, {
      type: 'examen', game: 'musica', category: 'musica', score: final.score, passed: final.superada,
      timeSpent: 0, coinsEarned: Math.min(Math.floor((final.score || 0) / 10), 200),
  ...
  if (pantalla === 'intro') {
    return (
      <div className="relative z-10 flex flex-col items-center min-h-[calc(100vh-4rem)] px-4 py-8">
        <SEOHead
          title={tr({ es: 'Examen de Música — Pentagrama Path', en: 'Music Exam — Pentagrama Path', ca: 'Examen de Música — Pentagrama Path' })}
          description={tr({ es: 'Examen de lectura de partituras por fases: de reconocer notas sin prisa a tocar a tempo real con clave de fa y alteraciones. Progresión graduada con nota final.', en: 'A graded sheet-music reading exam in phases: from note recognition at your own pace to real-tempo playing with bass clef and sharps.', ca: 'Examen de lectura de partitures per fases: de reconèixer notes sense pressa a tocar a tempo real amb clau de fa i alteracions.' })}
          path="/examen/musica" lang={lang} />
        <div className="max-w-md w-full">
          <button onClick={() => navigate(localPath('/juegos/pentagrama-path'))}
          ...
          <button onClick={() => navigate(localPath('/juegos/pentagrama-path'))}
            className="w-full text-left rounded-2xl border border-red-500/30 bg-gradient-to-r from-red-600/10 to-orange-600/10 p-4 mb-5 hover:from-red-600/20 hover:to-orange-600/20 transition-all">
            <div className="flex items-center gap-3">
              <span className="text-2xl">🔥</span>
              <div className="flex-1 min-w-0">
                <p className="text-white font-bold text-sm">{tr({ es: '¿Prefieres partitura infinita?', en: 'Prefer an endless score?', ca: 'Prefereixes partitura infinita?' })}</p>
                <p className="text-white/40 text-xs">{tr({ es: 'Prueba el Modo Survivor →', en: 'Try Survivor Mode →', ca: 'Prova el Mode Survivor →' })}</p>
              </div>
            </div>
          </button>
  ...
  if (pantalla === 'fin' && final) {
    ...
    return (
      <GameEndScreen
        ...
        stats={stats}
        shareText={tr({
          es: `He conseguido ${final.score} puntos en el examen de música de Tuthor 🎼 (${final.notaPct}% de notas correctas) — ¿puedes superarme? https://tuthor.es/examen/musica`,
          en: `I scored ${final.score} points in Tuthor's music exam 🎼 (${final.notaPct}% correct notes) — can you beat me? https://tuthor.es/examen/musica`,
          ca: `He aconseguit ${final.score} punts a l'examen de música de Tuthor 🎼 (${final.notaPct}% de notes correctes) — pots superar-me? https://tuthor.es/examen/musica`,
        })}
