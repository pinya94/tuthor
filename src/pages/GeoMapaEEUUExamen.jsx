...
function AutocompleteInput({ value, onChange, onSubmit, disabled, focusKey, lang }) {
  ...
  const nombres = lang === 'en' ? NOMBRES_ESTADOS : lang === 'ca' ? NOMBRES_ESTADOS_CA : NOMBRES_ESTADOS_ES
  ...
  return (
    <div className="relative">
      <input ref={ref} type="text" value={value}
        ...
        placeholder={lang === 'en' ? 'Type a state…' : lang === 'ca' ? 'Escriu un estat…' : 'Escribe un estado…'}
...
export default function GeoMapaEEUUExamen() {
  ...
  const pageMeta = <PageMeta
    title={en ? 'USA States Map Exam' : ca ? 'Examen Mapa dels EUA' : 'Examen Mapa de Estados Unidos'}
    description={en ? 'Identify US states on an interactive map. 10 questions, instant feedback.' : ca ? 'Identifica els estats dels EUA al mapa interactiu. 10 preguntes, retroalimentació immediata.' : 'Identifica los estados de EE.UU. en el mapa interactivo. 10 preguntas, feedback inmediato.'}
    path="/examen/geomapa-eeuu" lang={lang} />

  const pool = useMemo(() => shuffle(ESTADOS).slice(0, TOTAL), [])
  ...
  function getNombre(e) {
    return lang === 'en' ? e.nombre : lang === 'ca' ? e.nombreCa : e.nombreEs
  }
  ...
  function handleRespuesta(val) {
    ...
    if (match.nombre === estadoActual.nombre) {
      setAciertos(a => a + 1)
      setHistorial(h => [...h, { passed: true }])
      setFeedback({ ok: true, msg: `🎉 ${getNombre(estadoActual)}!` })
      setResueltoPais(true)
      setTimeout(() => siguienteEstado(), 1500)
      return
    }
    ...
    if (newErrores >= MAX_ERRORS) {
      ...
      setFeedback({ ok: false, msg: en ? `It was ${getNombre(estadoActual)}` : ca ? `Era ${getNombre(estadoActual)}` : `Era ${getNombre(estadoActual)}` })
      setResueltoPais(true)
      setTimeout(() => siguienteEstado(), 2000)
      return
    }
    setShowCapital(true)
    setFeedback({ ok: false, msg: en ? `Not that — capital: ${estadoActual.capital}` : ca ? `No és — capital: ${estadoActual.capital}` : `No es ese — capital: ${estadoActual.capital}` })
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[calc(100vh-4rem)] px-4 py-8">
        ...
        <div className="max-w-md w-full">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center mb-4">
            ...
            <p className="text-white/40 text-sm">{en ? 'States identified · United States' : ca ? 'Estats identificats · Estats Units' : 'Estados identificados · Estados Unidos'}</p>
          </div>
        ...
        <div className="bg-black/40 border border-white/10 rounded-2xl p-2 sm:p-3 mb-4">
          <div className="flex items-center justify-between mb-2 px-1">
            <h2 className="text-white/30 text-xs font-semibold uppercase tracking-widest">
              {en ? `State ${idx + 1}` : ca ? `Estat ${idx + 1}` : `Estado ${idx + 1}`}
            </h2>
            <span className="text-white/20 text-xs">{errores}/{MAX_ERRORS} {en ? 'errors' : 'errores'}</span>
          </div>
          <USAMap highlight={estadoActual.nombre} />
        </div>
        ...
        {showCapital && (
          <div className="flex gap-3 mb-4">
            <div className="flex-1 text-center py-3 rounded-xl border bg-amber-500/10 border-amber-500/30 text-white">
              ...
              <span className="text-sm font-bold text-white">{estadoActual.capital}</span>
