...
export default function GeometriaTema() {
  ...
  const metaTitle = en ? 'Geometry' : ca ? 'Geometria' : 'Geometría'
  const metaDesc = en
    ? 'Study Geometry on Tuthor: areas, perimeters, Pythagorean theorem, angles and volumes. Interactive practice for Primary and Secondary.'
    : ca
    ? 'Estudia Geometria a Tuthor: àrees, perímetres, teorema de Pitàgores, angles i volums. Pràctica interactiva per a Primària i ESO.'
    : 'Estudia Geometría en Tuthor: áreas, perímetros, teorema de Pitágoras, ángulos y volúmenes. Práctica interactiva para Primaria y ESO.'
  const pageMeta = <PageMeta title={metaTitle} description={metaDesc} path="/estudiar/matematicas/geometria" lang={lang} />
  const courseSchema = <CourseSchema name={metaTitle} description={metaDesc} path="/estudiar/matematicas/geometria" lang={lang} subject={en ? 'Mathematics' : 'Matemáticas'} />
  const breadcrumb = <BreadcrumbSchema lang={lang} items={[
    ...
    { name: metaTitle, path: '/estudiar/matematicas/geometria' },
  ]} />
