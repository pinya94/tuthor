...
export default function EstadosMateriaExamen() {
  ...
  const q = pool[idx]
  const backTo = backPath ? localPath(backPath) : localPath('/estudiar/quimica/estados-materia')
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[calc(100vh-4rem)] px-4 py-8">
        <div className="max-w-sm w-full text-center">
          ...
          <h1 className="text-white font-black text-2xl mb-2">
            {en ? 'States of Matter' : ca ? 'Estats de la Matèria' : 'Estados de la Materia'}
          </h1>
        ...
        <div className="max-w-md w-full">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center mb-4">
            ...
            <p className="text-white/40 text-sm">
              {en ? 'States of Matter' : ca ? 'Estats de la Matèria' : 'Estados de la Materia'} · {getText(NIVEL_INFO[nivelSel].label, lang)}
            </p>
          </div>
    ...
    <div className="relative z-10 flex flex-col min-h-[calc(100vh-4rem)] px-4 md:px-8 py-5 max-w-lg mx-auto w-full">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-white/40 text-sm font-bold">
          🧪 {en ? 'States of Matter' : ca ? 'Estats de la Matèria' : 'Estados de la Materia'} · {idx + 1}/{pool.length}
        </span>
      </div>
