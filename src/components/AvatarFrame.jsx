 ...
 */
export default function AvatarFrame({ user, frameId = 'default', size = 'lg' }) {
    ...
    >
      {user?.photoURL
        ? <img
          ...
          />
        : <div
            style={{ width: inner, height: inner, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#7c3aed', color: 'white', fontWeight: 900, fontSize: inner / 2.5 }}
          >
            {user?.displayName?.[0] ?? '?'}
          </div>
      }
