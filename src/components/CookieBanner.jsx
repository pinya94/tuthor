...
const CATEGORIES = [
  {
    id: 'essential',
    label: 'Esenciales',
    desc: 'Necesarias para que la app funcione: sesión de usuario, preferencias básicas. No se pueden desactivar.',
    required: true,
  },
  {
    id: 'analytics',
    label: 'Analítica',
    desc: 'Nos ayudan a entender cómo se usa Tuthor (Vercel Analytics). No recopilan datos personales identificables.',
    required: false,
  },
]
          ...
          {!showPrefs ? (
            <>
              ...
              <div className="flex items-start gap-3 mb-3">
                ...
                <div>
                  <h2 className="text-white font-bold text-base leading-tight">Usamos cookies</h2>
            ...
            <>
              ...
              <h2 className="text-white font-bold text-base mb-1">Preferencias de cookies</h2>
              <p className="text-white/40 text-xs mb-4">
                Puedes activar o desactivar cada categoría. Las cookies esenciales son siempre necesarias.
              </p>
                    ...
                    <button
                      ...
                      className={`mt-0.5 flex-shrink-0 w-10 h-5 rounded-full transition-all relative
                        ${cat.required
                          ...
                          : prefs[cat.id] ? 'bg-violet-600' : 'bg-white/10'}`}
                    >
                      <span
                        className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all
                          ${prefs[cat.id] ? 'left-5' : 'left-0.5'}`}
                      />
                    </button>
                  </div>
                ))}
              ...
              <p className="text-white/25 text-[10px] leading-relaxed mb-4">
                De acuerdo con el RGPD (UE) 2016/679 y la LSSI-CE, tienes derecho a retirar tu consentimiento en cualquier momento
                desde la configuración de la app. Para más información consulta nuestra{' '}
